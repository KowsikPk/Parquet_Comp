        ...
        <div className="flex items-center space-x-2">
          ...
          {totalMismatches > 0 ? (
            <div className="flex items-center space-x-1 text-[11px] font-mono">
              ...
              {removedCount > 0 && (
                <span className="px-1.5 py-0.5 rounded bg-red-500/15 text-red-400 font-bold border border-red-500/30">
        ...
        <div className="px-4 py-2 bg-theme-elevated/40 border-b border-theme-border text-xs flex flex-wrap gap-1.5 items-center">
          ...
          {colMismatches.map(diff => (
            <span key={diff.key} className="px-2 py-0.5 rounded text-[11px] font-mono bg-theme-base border border-theme-border">
              {diff.type === 'added' && <span className="text-emerald-400 font-bold">+ {diff.key}</span>}
              {diff.type === 'removed' && <span className="text-red-400 font-bold">- {diff.key}</span>}
              {diff.type === 'modified' && <span className="text-amber-400 font-bold">~ {diff.key}</span>}
            </span>
          ))}
                ...
                return (
                  ...
                  >
                    <div className="truncate pr-2">
                      ...
                      <span className={isDiff ? 'text-red-200 font-mono' : 'text-theme-text'}>
                        "{formatValue(valA)}"
                      </span>
                    </div>
                    {isRemoved && (
                      <span className="text-[9px] uppercase font-mono px-1 py-0.2 rounded bg-red-500/20 text-red-300 border border-red-500/40 shrink-0">
                        - REMOVED
                      </span>
                    )}
                    {isModified && (
                      <span className="text-[9px] uppercase font-mono px-1 py-0.2 rounded bg-amber-500/20 text-amber-300 border border-amber-500/40 shrink-0">
                        MODIFIED
                      </span>
                    )}
                  </div>
                      ...
                      className={`text-xs flex justify-between px-2 py-1 rounded font-mono ${
                        isDiff
                          ? type === 'added'
                            ...
                            : 'bg-sky-500/15 text-sky-300 font-bold border-l-2 border-sky-500'
                          : 'text-theme-text-secondary'
                      }`}
                    >
                      <span className="font-semibold">{key}:</span>
              ...
              {filteredPropDiffs.map(({ key, type, valB }) => {
                ...
                return (
                  ...
                  >
                    <div className="truncate pr-2">
                      <span className={isDiff ? 'font-bold' : 'text-theme-text-secondary'}>{key}: </span>
                      <span className={isDiff ? 'text-emerald-200 font-mono' : 'text-theme-text'}>
                        "{formatValue(valB)}"
                      </span>
                    </div>
                    {isAdded && (
                      <span className="text-[9px] uppercase font-mono px-1 py-0.2 rounded bg-emerald-500/20 text-emerald-300 border border-emerald-500/40 shrink-0">
                        + ADDED
                      </span>
                    )}
                    {isModified && (
                      <span className="text-[9px] uppercase font-mono px-1 py-0.2 rounded bg-sky-500/20 text-sky-300 border border-sky-500/40 shrink-0">
                        MODIFIED
                      </span>
                    )}
                  </div>
      ...
      {viewMode === 'unified' && (
        <div className="p-4 bg-theme-base/90 font-mono text-xs leading-relaxed max-h-96 overflow-y-auto divide-y divide-theme-border/20">
          ...
          {colDiffs.map(({ key, type, valA, valB }) => {
            ...
            if (type === 'removed') {
              return (
                <div key={key} className="py-0.5 bg-red-500/15 text-red-300 pl-4 font-bold border-l-2 border-red-500">
                  - {key}: {formatValue(valA)}
                </div>
              );
            }
            if (type === 'added') {
              return (
                <div key={key} className="py-0.5 bg-emerald-500/15 text-emerald-300 pl-4 font-bold border-l-2 border-emerald-500">
            ...
            return (
              <React.Fragment key={key}>
                <div className="py-0.5 bg-red-500/15 text-red-300 pl-4 font-bold border-l-2 border-red-500">
                  - {key}: {formatValue(valA)}
                </div>
                <div className="py-0.5 bg-emerald-500/15 text-emerald-300 pl-4 font-bold border-l-2 border-emerald-500">
                  + {key}: {formatValue(valB)}
                </div>
              </React.Fragment>
            );
          ...
          {filteredPropDiffs.map(({ key, type, valA, valB }) => {
            ...
            if (type === 'removed') {
              return (
                <div key={key} className="py-0.5 bg-red-500/15 text-red-300 pl-6 font-bold border-l-2 border-red-500 flex items-center justify-between">
                  <span>- {key}: "{formatValue(valA)}"</span>
                  <span className="text-[9px] uppercase px-1 rounded bg-red-500/20 text-red-300">deleted</span>
                </div>
              );
            }
            if (type === 'added') {
              return (
                <div key={key} className="py-0.5 bg-emerald-500/15 text-emerald-300 pl-6 font-bold border-l-2 border-emerald-500 flex items-center justify-between">
                  <span>+ {key}: "{formatValue(valB)}"</span>
                  <span className="text-[9px] uppercase px-1 rounded bg-emerald-500/20 text-emerald-300">added</span>
                </div>
              );
            }
            // Modified
            return (
              <React.Fragment key={key}>
                <div className="py-0.5 bg-red-500/15 text-red-300 pl-6 font-bold border-l-2 border-red-500 flex items-center justify-between">
                  <span>- {key}: "{formatValue(valA)}"</span>
                  <span className="text-[9px] uppercase px-1 rounded bg-red-500/20 text-red-300">old value</span>
                </div>
                <div className="py-0.5 bg-emerald-500/15 text-emerald-300 pl-6 font-bold border-l-2 border-emerald-500 flex items-center justify-between">
                  <span>+ {key}: "{formatValue(valB)}"</span>
                  <span className="text-[9px] uppercase px-1 rounded bg-emerald-500/20 text-emerald-300">new value</span>
                </div>
              </React.Fragment>
            );
          })}
